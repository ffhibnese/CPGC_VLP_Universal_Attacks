...
class CLIP(nn.Module):
    ...
    def encode_text(self, text, uap_embedding=None, position=None):
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]
        if uap_embedding is not None:
            for i in range(len(position)):
                for j in range(uap_embedding.size(0)):
                    x[i][position[i][j]] = uap_embedding[j]
    ...
    def inference_text_replace(self, text_input, uap_embedding, position, padding_zero):
        text = []
        for input_ids in text_input.input_ids:
            t = self.tokenizer.decode(input_ids).replace('[PAD]', '').replace('[CLS]', '').replace('[SEP]', '').strip()
            text.append(t)
        text_input = clip.tokenize(text, 77, True).to(self.logit_scale.device)
        txt_embed = self.encode_text(text_input, uap_embedding, position)
        text_feat = F.normalize(txt_embed, dim=-1)
        return {'text_embed': txt_embed,
                'text_feat': text_feat,}
