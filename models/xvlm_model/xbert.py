...
class BertEmbeddings(nn.Module):
    ...
    def forward(
        self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, past_key_values_length=0, replace_embedding=None, replace_position=None, padding_zero=None
    ):
        ...
        if inputs_embeds is None:
            ...
            if replace_position is not None:
                ...
                zero_embedding = self.word_embeddings(zero)
                for i in range(len(replace_position)):
                    inputs_embed = inputs_embeds[i]
                    for j in range(len(replace_position[i])):
                        inputs_embed[j] = replace_embedding[j]
                        if (padding_zero[i][j] > 0) and (replace_position[i][j]+padding_zero[i][j] < seq_length):
                            inputs_embed_clone = inputs_embed.clone()
                            inputs_embed[replace_position[i][j]+1: -padding_zero[i][j]] = inputs_embed_clone[replace_position[i][j]+padding_zero[i][j]+1:]
                    inputs_embed[-sum(padding_zero[i]):] = zero_embedding
                    inputs_embeds[i] = inputs_embed
