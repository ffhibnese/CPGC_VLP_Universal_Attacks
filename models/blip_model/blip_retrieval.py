from models.blip_model.med import BertConfig, BertModel
...
from models.blip_model.blip import create_vit, init_tokenizer, load_checkpoint

class BLIP_Retrieval(nn.Module):
    def __init__(self,                
                 med_config = 'models/blip_model/med_config.json',  
                 image_size = 384,
                 vit = 'base',
                 vit_grad_ckpt = False,
                 vit_ckpt_layer = 0,                      
    ...
    def inference_image(self, image):
        ...
        image_embeds = self.visual_encoder(image)    
        image_feat = F.normalize(self.vision_proj(image_embeds[:,0,:]),dim=-1)  
        return {'image_embed': image_embeds,
    ...
    def inference_text_replace(self, text_input, embedding, position, padding_zero):
        ...
        text_output = self.text_encoder(text_input.input_ids, attention_mask=text_input.attention_mask,
                                        mode='text', replace_embedding = embedding, replace_position = position, padding_zero = padding_zero)
